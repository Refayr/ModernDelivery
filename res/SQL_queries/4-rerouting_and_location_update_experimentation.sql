...
WITH directional AS (
    ...
    -- 1) current position + destination + heading for ALL ships
    SELECT
        sl.imo,
...
valid_moves AS (
    ...
    WHERE NOT EXISTS (
        SELECT 1
        FROM continents land
        WHERE ST_Intersects(c.next_pos, land.geom)
    )

)

-- 4) insert new trajectory points (ALL ships)
INSERT INTO ship_locations (
    imo,
    updated_at,
    latitude,
